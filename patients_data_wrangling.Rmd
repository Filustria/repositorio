...
consteste <- base %>% select(PACIENTE_ID, ATENDIMENTO_ID)

cadsteste <- banco31 %>% select(PACIENTE_ID, NOME_UNIDADE, MICRO_AREA)

cadsteste <- cadsteste %>% distinct()

mmm <- merge(consteste, cadsteste, by = "PACIENTE_ID", all = TRUE)

mm <- mmm %>% distinct()

mm$atendimento <- as.numeric(ifelse(is.na(mm$ATENDIMENTO_ID), 0, 1))

sumario_de_consultas <- mm %>% select(PACIENTE_ID, atendimento, MICRO_AREA) %>% 
  group_by(PACIENTE_ID) %>% summarise(consultas = sum(atendimento))

teste <- merge(sumario_de_consultas, cadsteste, by = "PACIENTE_ID", all.x = TRUE)


gg <- teste %>% 
  group_by(NOME_UNIDADE, MICRO_AREA) %>% 
  summarise(cadastrados = n(),
            consultou = sum(consultas != 0),
            no_consultou = sum(consultas == 0)) %>%
  mutate(prop_com = consultou/cadastrados,
         prop_sem = no_consultou/cadastrados)

ggg <- gg %>% group_by(NOME_UNIDADE) %>% summarise(total = cadastrados)

pacs <- mm %>% group_by(PACIENTE_ID, MICRO_AREA) %>% count(atendimento)

pacss <- pacs %>% group_by(MICRO_AREA) %>% summarise(max = max(n))

ga <- teste %>% filter(is.na(MICRO_AREA)) %>% group_by(NOME_UNIDADE) %>% summarise(max = max())

ver <- banco31b %>% filter(PACIENTE_ID == "ADD2651D-9DFC-E811-8167-1C1B0DE65C14")

outro <- banco31b %>% filter(PACIENTE_ID == "97B7651D-9DFC-E811-8167-1C1B0DE65C14")

profs <- ver %>% group_by(NOME_PROF) %>% count()

## verificar se os pacientes que tem duplo cadastro tem as consultas antes ou depois.
...
# Data da primeira consulta em cada cl?nica
...
# Diniz recebeu os pacientes do parque uni?o e algumas equipes do H?lio smidt
...
consultas$MFCcat <- ifelse(consultas$TIPO_ATENDIMENTO == "Consulta de Enfermagem", "Nurse",
                           ifelse(consultas$TIPO_ATENDIMENTO == "Consulta de Sa?de Bucal", "Dentist",
                                  ifelse(consultas$TIPO_ATENDIMENTO == "Consulta M?dica" & consultas$MFC == 1, 
                                         "FP", "Generalist")))
...
consultas <- consultas %>% filter(TIPO_ATENDIMENTO == "Consulta M?dica")
...
# Prescri??o de benzodiazep?nicos
...
cadastrados$NOME_PAC[cadastrados$NOME_PAC == "A?UCENA KELLY SILVA"] <- "ACUCENA KELY SILVA SERRA"
...
cadastrados$NOME_MAE[cadastrados$NOME_PAC == "ADAILZA MARIA BITTENCOURT DOS REIS"] <- "ANISIA LUIZA BITTENCOURT DOS REIS"
...
iniciais1 <- LETTERS[seq( from = 1, to = 10 )]

nomes_a_corrigir1 <- cadastrados %>% 
  select(PACIENTE_ID, cluster_nome, cluster_ID, cluster_cadastro, NOME_PAC, 
         NOME_MAE, DATA_NASC, SEXO, micro, TIPO_LOGRADOURO_ENDERECO, 
         NOME_LOGRADOURO, NUM_ENDERECO, BAIRRO_ENDERECO, 
         NOME_UNIDADE, DATA_INSCRICAO, SITUACAO_PACIENTE, MICRO_AREA) %>%
  filter(grepl(paste(iniciais1, collapse = "|"), substring(NOME_PAC, 1, 1)))

iniciais2 <- LETTERS[seq( from = 11, to = 26 )]


nomes_a_corrigir2 <- cadastrados %>% 
  select(PACIENTE_ID, cluster_nome, cluster_ID, cluster_cadastro, NOME_PAC, 
         NOME_MAE, DATA_NASC, SEXO, micro, TIPO_LOGRADOURO_ENDERECO, 
         NOME_LOGRADOURO, NUM_ENDERECO, BAIRRO_ENDERECO, 
         NOME_UNIDADE, DATA_INSCRICAO, SITUACAO_PACIENTE, MICRO_AREA) %>%
  filter(grepl(paste(iniciais2, collapse = "|"), substring(NOME_PAC,1,1)))


write.table(nomes_a_corrigir1, "C:\\Users\\Adelson\\Desktop\\pendrive\\bancos_workspace\\nomes_a_corrigir1.csv", 
            row.names = FALSE, sep = ";", dec = ",")
write.table(nomes_a_corrigir2, "C:\\Users\\Adelson\\Desktop\\pendrive\\bancos_workspace\\nomes_a_corrigir2.csv", 
            row.names = FALSE, sep = ";", dec = ",")
...
doencas <- consult[,c(75:77, 16, 25:74)] # aqui precisa pegar a vari?vel "cluster_ID"
...
# Aqui precisa fazer com que todas as datas estejam presentes para determinar quando o paciente come?a a contar o tempo e quando deixa de contar.
...
# 886.356 pacientes com ao menos uma informa??o de data
cadastros <- cadastros %>% 
